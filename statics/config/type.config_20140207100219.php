...
$querytype = array(
		'user' =>array(
			...
			'child' => array(
				'user_number' => array(
               	...
               	'com_news' => array(
						'name' => '企业新闻数量' ,
						'sql' => "select '新闻数量', count(*) from gongchanginfo.gc_comnews where cid > 0 and is_top > 0 and pubtime > {starttime}  and pubtime < {endtime}"
					),
               'com_job' => array(
						'name' => '企业招聘数量' ,
						'sql' => "select '招聘数量', count(*) from gongchanginfo.gc_comjob where  cid > 0 and is_top > 0 and pubtime > {starttime}  and pubtime < {endtime}"
					),
			    'com_merchant' => array(
						'name' => '企业招商数量' ,
						'sql' => "select '招商数量', count(*) from gongchanginfo.gc_merchant where cid > 0 and is_top > 0 and pubtime > {starttime}  and pubtime < {endtime}"
					),
				'com_msg' => array(
						'name' => '商铺留言数量' ,
						'sql' => "select '商铺留言数量', count(*) from gongchanginfo.gc_websitemsg where cid > 0 and msgtype > 0 and pubtime > {starttime}  and pubtime < {endtime}"
					),
		...
		'usertake' =>array(
			...
			'child' => array(
				...
				'com_buyfavourite' => array(
				 ...
				 'com_buyfeed' => array(
						'name' => '采购订阅数量' ,
						'sql' => "select '采购订阅数量', count(*) from gongchanginfo.gc_feed  where uid > 0 and type = 2 and addtime > {starttime}  and addtime < {endtime} "
					),
				 'com_profeed' => array(
						'name' => '产品订阅数量' ,
						'sql' => "select '产品订阅数量', count(*) from gongchanginfo.gc_feed  where uid > 0 and type = 1 and addtime > {starttime}  and addtime < {endtime} "
					),
     			 'com_msgksh' => array(
						'name' => '资讯矿山机械数量' ,
						'sql' => "select '资讯矿山机械数量', count(*) from gongchanginfo.gc_feed  where uid > 0 and type = 1 and subkey = 1 and addtime > {starttime}  and addtime < {endtime} "
					),
	   			 'com_msgcar' => array(
						'name' => '资讯汽车数量' ,
						'sql' => "select '资讯汽车数量', count(*) from gongchanginfo.gc_feed  where uid > 0 and type = 4 and subkey = 2 and addtime > {starttime}  and addtime < {endtime} "
					),
	   			 'com_msgjiancai' => array(
						'name' => '资讯建材数量' ,
						'sql' => "select '资讯建材数量', count(*) from gongchanginfo.gc_feed  where uid > 0 and type = 4 and subkey = 3 and addtime > {starttime}  and addtime < {endtime} "
					),
			     'com_msgzhuangbei' => array(
						'name' => '资讯装备制造数量' ,
						'sql' => "select '资讯装备制造数量', count(*) from gongchanginfo.gc_feed  where uid > 0 and type = 4 and subkey = 4 and addtime > {starttime}  and addtime < {endtime} "
					),
			     'com_msgshipin' => array(
						'name' => '资讯食品数量' ,
						'sql' => "select '资讯食品数量', count(*) from gongchanginfo.gc_feed  where uid > 0 and type = 4 and subkey = 5 and addtime > {starttime}  and addtime < {endtime} "
					),
				  'com_msgnongye' => array(
						'name' => '资讯现代农业数量' ,
						'sql' => "select '资讯现代农业数量', count(*) from gongchanginfo.gc_feed  where uid > 0 and type = 4 and subkey = 6 and addtime > {starttime}  and addtime < {endtime} "
					),
				  'com_msgfuzhuang' => array(
						'name' => '资讯纺织服装数量' ,
						'sql' => "select '资讯纺织服装数量', count(*) from gongchanginfo.gc_feed  where uid > 0 and type = 4 and subkey = 7 and addtime > {starttime}  and addtime < {endtime} "
					)
				)
		),
		'offer' => array(
		...
		'comtype' => array(
				...
				'com_busmode' => array(
					...
					'audit_company_true' => array(
						'name' => '审核通过的企业' ,
						'sql' => "select a.busmode ,count(*) from gongchanginfo.gc_company a where a.addtime > {starttime} and a.addtime < {endtime} and a.status = 1 group by a.busmode order by a.busmode"
						),

					'audit_company_false' => array(
						'name' => '被拒审的企业' ,
						'sql' => "select busmode ,count(*) from gongchanginfo.gc_company a where a.addtime > {starttime} and a.addtime < {endtime} and a.status = -1 group by a.busmode order by a.busmode"
						) 
					),
	...
	'industry' => array(
				...
				'child' => array(
					...
					'auditcom2' => array(
						'name' => '认证用户' ,
						'sql' => "select cate1 ,count(*) from gongchanginfo.gc_company a , gongchanginfo.gc_userinfo b  where a.uid = b.uid and a.addtime > {starttime} and  a.addtime < {endtime} and b.usergroup = 3  group by a.cate1 order by cate1",
						) ,

					'gongchanguser' => array(
						'name' => '工厂用户' ,
						'sql' => "select cate1 ,count(*) from gongchanginfo.gc_company a , gongchanginfo.gc_userinfo b where a.uid = b.uid and a.addtime > {starttime} and  a.addtime < {endtime} and  b.usergroup = 4 group by a.cate1 order by cate1",
						),

					'payuser' => array(
						'name' => '付费用户' ,
						'sql' => "select cate1 ,count(*) from gongchanginfo.gc_company a , gongchanginfo.gc_userinfo b where a.uid = b.uid and a.addtime > {starttime} and  a.addtime < {endtime} and  b.usergroup = 5 group by a.cate1 order by cate1",
						)
				),
   	...
   	'gc_mobile' => array(
				...
				'child' => array(
					...
					'mobile_shopnum' => array(
						'name' => '开通商铺新会员' ,
						'sql' => "select '开通商铺新会员',count(*) from gongchanginfo.gc_userinfo where regtime > {starttime} and regtime < {endtime} and is_mobile = 1 and comstatus > 0 ",
						),
